...
import asyncio, csv, io, json, os, random, threading, time
import pandas as pd
...
async def _collect(session, sem, slug, title, year):
    ...
    while not should_stop.is_set():
        ...
        p(f"    [{title}] 페이지 {page}~{page + pages_per_req - 1}: 누적 {len(reviews)}개")
        ...
        for i in range(1, RETRY_COUNT + 1):
            ...
            try:
                ...
                async with save_lock:
                    ...
                    p(f"    -> '{title}' {len(reviews)}개 저장 완료 (이번 세션: {total_saved}개 / 전체: {total_in_file:,}개)")
                    if total_in_file // 50000 > prev_total // 50000:
                        backup_csv(total_in_file)
                return
            except Exception as e:
                p(f"    -> '{title}' 수집 실패 ({i}/{RETRY_COUNT}): {e}")
                if i == RETRY_COUNT:
                    p(f"    -> '{title}' 건너뜀")
                    already_done.add(title)
                else:
